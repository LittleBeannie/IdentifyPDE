...
x1 = seq(0.1, 0.3, length.out = M)#seq(0.1, 0.3, length.out = M)
x2 = seq(0.2, 0.4, length.out = M)#seq(0.2, 0.4, length.out = M)
x = NULL
for (i in 1:M) {
  for (j in 1:M) {
...
##########################################################################
data.plot.true = list( x = x1, y = x2, z = u.true[,,1] )
data.plot.obs = list( x = x1, y = x2, z = u.obs[,,1] )
data.plot.est = list( x = x1, y = x2, z = u.obs[,,1] )
fig <- plot_ly(x = data.plot$x, y = data.plot$y, z = data.plot.true$z) %>% add_surface()
fig <- fig %>% add_surface(z = data.plot.obs$z, opacity = 0.98)
fig
